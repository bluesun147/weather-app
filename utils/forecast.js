...
const forecast = (latitude, longitude, callback) => {
    const url1 = `http://api.weatherstack.com/current?access_key=7a6b5de2e07356b5635da2f06d6495f2&query=${longitude},${latitude}`;
    ...
    request({url: url1, json: true}, (error, response) => { // 둘중 하나만. 하나는 null
        ...
        if (error) { // low level error like no network connection
            ...
            const currD = response.body.current.temperature; // 현재 온도
            const feelslikeD = response.body.current.feelslike; // 체감 온도
            callback(undefined, `${response.body.current.weather_descriptions[0]}, It is currently ${currD} degree, feels like ${feelslikeD} degree out.`);
        }
    })
}
